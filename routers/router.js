...
router.post("/register", function (req, res) {
  // 验证格式
  if (
    mailReg.test(req.body.username) &&
    ...
    if (req.body.code === AES.get(req.cookies.wn)) {
      ...
      fs.mkdir(`${userPath}/${req.body.username}`, (err) => {
        if (err) {
          fs.readFile(
            `${userPath}/${req.body.username}/info/userInfo.json`,
            "utf-8",
            (err, data) => {
              if (err) {
                ...
                if (
                  ...
                  req.body.password === userinfo.psw
                ) {
          ...
          fs.cp(
            mdPath,
            `${userPath}/${req.body.username}/md`,
            { recursive: true },
            function (err) {
              if (err) {
                errNum++;
              } else {
                fs.mkdir(`${userPath}/${req.body.username}/img`, (err) => {
                  if (err) {
                    errNum++;
                  }
                });
                fs.mkdir(`${userPath}/${req.body.username}/info`, (err) => {
                  if (err) {
                    errNum++;
                  }
                });
                ...
                fs.writeFile(
                  `${userPath}/${req.body.username}/info/log.txt`,
                  ``,
                  { flag: "a" },
                  (err) => {
                    if (err) {
      ...
      AES.get(req.cookies.wn) === undefined
        ? res.send({ code: 0, msg: "验证码已过期。" })
...
router.get("/topNav", (req, res) => {
  fs.readdir(
    req.cookies &&
      ...
      fs.statSync(`${userPath}/${AES.get(req.cookies.webnote)}/md`, {
    ...
    (err, files) => {
      err
        ? res.send({ code: 0, data: ["请刷新重试。", "意料之外的错误，"] })
  ...
  fs.readdir(
    ...
    (err, data) => {
      if (err || !fs.statSync(tmpPath, { throwIfNoEntry: false })) {
        res.send({
          code: 0,
          data: [{ title: "404", value: ["意料之外的错误，", "请刷新重试。"] }],
        });
      } else {
        let tmpArr = [];
        let tmpObj = {};
...
router.post("/delFolder", function (req, res) {
  if (
    ...
    fs.rm(
      ...
      (err) => {
        err
          ? res.send({ code: 0, msg: "意料之外的错误，请刷新页面重试。" })
...
router.post("/setMdName", function (req, res) {
  ...
  fs.access(`${path}/${req.body.mdName}.md`, fs.constants.F_OK, (err) => {
    if (err) {
      ...
      fs.rename(
        `${path}/${req.body.mdName}.md`,
        `${path}/${req.body.newMdName}.md`,
        (err) => {
          if (err) {
            res.send({
              code: 0,
              msg: `${req.body.mdName} 重命名失败，请稍后重试`,
            });
          } else {
            res.send({
              code: 1,
              msg: `${req.body.mdName} 已重命名为 ${req.body.newMdName} `,
            });
          }
        }
      );
  ...
  if (cpType(req, `${userPath}/${AES.get(req.cookies.webnote)}`)) {
    if (req.body.imgNameArr instanceof Array) {
      ...
      req.body.imgNameArr.forEach((item) => {
        if (
          !fs.statSync(
            `${userPath}/${AES.get(req.cookies.webnote)}/img/${item}`,
            { throwIfNoEntry: false }
          )
        ) {
          imgArr.push(item);
      ...
      imgArr.length === 0
        ? res.send({ code: 1, msg: "全部存在。", data: [] })
        : res.send({ code: 0, msg: "msg中的不存在", data: imgArr });
    } else {
...
router.post("/cookieType", function (req, res) {
  fs.readdirSync(userPath).includes(AES.get(req.cookies.webnote))
...
//  摧毁 cookie
router.get("/unCookies", (req, res) => {
  setCookie(res, "webnote", "", 0);
  setCookie(res, "wn", "", 0);
  res.send();
});
...
function cpType(req, paths) {
  return (
    !!AES.get(req.cookies.webnote) &&
    !!fs.statSync(paths, { throwIfNoEntry: false })
  );
}
